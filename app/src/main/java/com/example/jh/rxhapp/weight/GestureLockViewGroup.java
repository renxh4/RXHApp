...
public class GestureLockViewGroup extends RelativeLayout {
     ...
     * 设置手势密码
     */
    public static final int SET_GESTURES = 0;
    /**
     * 验证手势密码
     */
    public static final int VERYFI_GESTURES = 1;
    /**
     * 当前模式
     */
    public static int CURRENT_MODEL = SET_GESTURES;
    /**
                ...
                if (child != null) {
                    switch (CURRENT_MODEL) {
                        case SET_GESTURES:
                            setGesturePwd(child);
                            break;
                        case VERYFI_GESTURES:
                            checkGesturePwd(child);
                            break;
                        default:
                    }
                }
    ...
    private void savePwd() {
        // 回调是否成功
        if (mOnGestureLockViewListener != null && mAnswers.size() > 0) {
            mOnGestureLockViewListener.isSetGestureSucess(true);
        } else {
            mOnGestureLockViewListener.isSetGestureSucess(false);
        }
    }

    private void chackPwd() {
        this.mTryTimes--;
    ...
    private void setGesturePwd(GestureLockView child) {
        int cId = child.getId();
        if (!mAnswers.contains(cId)) {
            mAnswers.add(cId);
    ...
    private void checkGesturePwd(GestureLockView child) {
        int cId = child.getId();
    ...
    private void changeItemMode() {
        for (GestureLockView gestureLockView : mGestureLockViews) {
            if (mChoose.contains(gestureLockView.getId())) {
                ...
                gestureLockView.setSuccess(isSuccess);
    ...
    private void reset() {
        ...
        for (GestureLockView gestureLockView : mGestureLockViews) {
            ...
            gestureLockView.setSuccess(false);
        }
    }
    ...
    private boolean checkAnswer() {
        if (mAnswers.size() != mChoose.size())
            return false;

        for (int i = 0; i < mAnswers.size(); i++) {
            if (!mAnswers.get(i).equals(mChoose.get(i)))
                return false;
        }
