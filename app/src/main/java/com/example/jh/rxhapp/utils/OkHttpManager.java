...
public class OkHttpManager {
    ...
    private static OkHttpManager mInstanceSimaple;
    ...
    private OkHttpManager(boolean boo) {
        mOkHttpClient1 = new OkHttpClient.Builder()
                .connectTimeout(60, TimeUnit.SECONDS)
                .writeTimeout(60, TimeUnit.SECONDS)
                .readTimeout(60, TimeUnit.SECONDS)
    ...
    public static OkHttpManager getInstanceSimaple(Context context) {
        if (mInstanceSimaple == null) {
            synchronized (OkHttpManager.class) {
                if (mInstanceSimaple == null) {
                    mInstanceSimaple = new OkHttpManager(true);
                }
            }
        }
        return mInstanceSimaple;
    }

    public Response getAsyn(String url) {
        Response mExecute = null;
        final Request request = new Request.Builder()
                .url(url)
                .build();
        try {
            mExecute = mOkHttpClient1.newCall(request).execute();
        } catch (IOException e) {
            e.printStackTrace();
        }
        return mExecute;
    ...
    private void _AsynAddheader(String url, final ResultCallback callback) {
        final Request request = new Request.Builder()
                .url(url)
                .addHeader("eSupport_SessionID", sLenovosessionid)
    ...
    private void _PostForm(String url, final ResultCallback callback, String messageid, String subject, String body, File file, String tag, String isEmail) {
        ...
        if (file != null) {
            ...
            requestBody = new MultipartBody.Builder()
                    ...
                    .addPart(
                            Headers.of("Content-Disposition", "form-data; name=\"optInEmail\""),
